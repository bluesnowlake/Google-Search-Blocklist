...
blocklist.manager.createBlocklistPattern = function(pattern) {
  ...
  var editBtn = $('<a href="#" class="manager-btn"></a>')
      .appendTo(operTd);
  ...
  deleteBtn.click(function() {
    ...
    if ($(this).text() == chrome.i18n.getMessage('unblock')) {
      chrome.extension.sendRequest({type: blocklist.common.DELETEFROMBLOCKLIST,
        pattern: pattern},
...
blocklist.manager.handleDeleteBlocklistResponse = function(response) {
  if (response.success) {
    chrome.tabs.getSelected(null, function(tab) {
      chrome.tabs.sendRequest(tab.id, {type: 'refresh'});
    });
  }
};
...
blocklist.manager.importPatterns = function() {
  ...
  if (patterns.length) {
    chrome.extension.sendRequest({type: blocklist.common.ADDBULKTOBLOCKLIST,
                                  patterns: patterns},
                                 blocklist.manager.handleImportResponse);
  } else {
...
blocklist.manager.showExportArea = function() {
  chrome.extension.sendRequest({'type': blocklist.common.GETBLOCKLIST},
                               blocklist.manager.handleExportListRequest);
};
